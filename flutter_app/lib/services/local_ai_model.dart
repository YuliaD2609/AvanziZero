...
class LocalPredictiveModel {
  ...
  static Map<String, List<ItemModel>> predict(
    ...
    Set<String> shoppingNames = shoppingItems.map((s) => s.name.toLowerCase().trim()).toSet();

    // 1. GESTIONE SCADENZE (urgencyLevel == 2)
    for (var item in pantryItems) {
      if (item.urgencyLevel == 2 && !shoppingNames.contains(item.name.toLowerCase().trim())) {
    ...
    for (var log in consumptionHistory) {
      ...
      consumptionScores[nameKey] = (consumptionScores[nameKey] ?? 0.0) + logScore;
      
      // Cerchiamo di dedurre la categoria basandoci sulla dispensa attuale se esiste
      var match = pantryItems.where((i) => i.name.toLowerCase().trim() == nameKey);
      if (match.isNotEmpty) {
    ...
    consumptionScores.forEach((nameKey, score) {
      ...
      var pantryMatch = pantryItems.where((i) => i.name.toLowerCase().trim() == nameKey);
      int currentQty = pantryMatch.isNotEmpty ? pantryMatch.first.quantity : 0;
      String category = itemCategories[nameKey] ?? 'Altro';
      String originalName = pantryMatch.isNotEmpty ? pantryMatch.first.name : nameKey;
      ...
      double safetyThreshold = 1.5 * groupSize; 
      
      if (currentQty == 0) {
        // Esaurito ma consumato in passato
        scarcity.add(ItemModel(id: '', name: originalName, expireDate: 'Data: N/A', quantity: 1, category: category));
      } else {
        ...
        if (scarcityRatio > 0.8 || (currentQty <= 2 && score >= 1.0)) {
          ...
          bool isExpiring = expiring.any((e) => e.name.toLowerCase().trim() == nameKey);
          if (!isExpiring) {
            scarcity.add(ItemModel(id: '', name: originalName, expireDate: 'Data: N/A', quantity: 1, category: category));
          }
        }
